    ...
    //09173145734
    //Changes to this release was a minor bug fix for DTMF TX enable control
    //Additional dtmf_mute signal from Pin 7 (active high) 
    ...
    long trigger_timeout = 90; // maximum trigger time, seconds
    long TimeoutTime = 0;
    long time_var = 0;
    ...
    int dtmf_detected = 0;
    int tx_enable = 1;
    int hangtime = 640; // time adjust for tx off delay before beep, msec
    #include <DTMF.h>

  const int sensorPin = A0;
  const int dtmf_led = 9;
  const int dtmf_mute = 7; //for dtmf mute relay/switch control signal
  char thischar;
  float n=128.0;
  // sampling rate in Hz
...
void trigger_control() {
  ...
  if (digitalRead(rx_signal) == 1 && tx_enable == 0)  {
    readString="";        
  }
  //clear DTMF String on RX stanby 
