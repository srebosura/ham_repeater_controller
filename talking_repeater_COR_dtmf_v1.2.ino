...
//Samuel Rebosura
//with featured Text to voice module and DS3231 RTC
...
void trigger_control() {
  ...
  if (digitalRead(rx_signal) == 1 && tone_ready == 1)  {
    ...
    delay(250);
      ...
      if (tone_enable == 0) {
      voice_msg("Beep disabled.");
      }
      if (tone_enable == 1) {
      voice_msg("Beep enabled.");
      }
      if (tone_enable == 2) {
...
void tone2() {
      tone(tone_out, 784, 150);
      delay(100);
      tone(tone_out, 587, 200);
      delay(200);
      tone_ready = 0;
 ...
 void set_timeout() {
  ...
  if (readString.length() == 4) {
    ...
    if (readString == "4321") {
       ...
       delay (1000);
       voice_out("Anteqera Radio and Cellphone Communication Incorporated");  
    }
    if (readString == "5930") {
       delay (1000);
       emicSerial.print("N2");
...
void check_time ()
{
 ...
 if (hrString == "06:00:00") { 
  voice_out("Good Morning. Time is six o'clock.");
 }
 if (hrString == "09:00:00") { 
  voice_out("It's nine A,M.");
 }
 if (hrString == "12:00:00") { 
  voice_out("It's twelve o'clock. Good noon.");
 }
 if (hrString == "15:00:00") { 
  voice_out("Time check. It's Three PM.");
 }
 if (hrString == "18:00:00") { 
  voice_out("Time is six in the evening.");
 }
